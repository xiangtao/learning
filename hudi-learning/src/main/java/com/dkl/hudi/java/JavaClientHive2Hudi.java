...
/**
 * Hudi Java Client 代码示例 (基于Hudi 0.12.0) 实现读取Hive表转化为Hudi表,同步Hive元数据实现自动建表 支持insert/upsert/delete
...
public class JavaClientHive2Hudi {
    ...
    private static String HIVE_JDBC_URL = "jdbc:hive2://172.19.17.107:10000/test_hudi";
    ...
    private static String tablePath = "/tmp/test_hudi/test_hudi_target";
    ...
    /**
     * 创建Hive表模拟历史数据转化 create database test_hudi;
     * <p>
     * create table test_hudi.test_source ( id int, name string, price double, dt string, ts bigint
     * );
     * <p>
...
//                            .bloomIndexPruneByRanges(false) // 1000万总体时间提升1分钟
            ...
            if (writeOperationType.equals(DELETE_OPERATION)) {
                writeClient.delete(getKeysForDelete(selectTable(options), writeSchema),
                    newCommitTime);
            } else {
                List<HoodieRecord<HoodieRecordPayload>> records =
                    generateRecord(selectTable(options), writeSchema, payloadClassName,
                        shouldCombine);
            ...
            //同步Hive元数据
            URL hiveSiteUrl = HiveConf.class.getClassLoader()
                .getResource(hiveSitePath);
            HiveConf hiveConf = getHiveConf(hiveSiteUrl.getPath(), coreSitePath, hdfsSitePath);
    ...
    /**
     * 利用HiveSyncTool同步Hive元数据 Spark写Hudi同步hive元数据的源码就是这样同步的
     *
     * @param properties
     * @param hiveConf
     */
    ...
    public static HiveConf getHiveConf(String hiveSitePath, String coreSitePath,
        String hdfsSitePath) {
    ...
    public static List<HoodieRecord<HoodieRecordPayload>> generateRecord(ResultSet rs,
        ...
        while (rs.next()) {
            ...
            writeSchema.getFields().forEach(field -> {
                try {
                    rec.put(field.name(),
                        convertValueType(rs, field.name(), field.schema().getType()));
            ...
            String partitionPath =
                partitionFields == null ? "" : getRecordPartitionPath(rs, writeSchema);
            System.out.println(partitionPath);
            String rowKey = recordKeyFields == null && writeOperationType.equals(INSERT_OPERATION)
                ? UUID.randomUUID().toString() : getRecordKey(rs, writeSchema);
     ...
     * @throws SQLException
     */
    private static List<HoodieKey> getKeysForDelete(ResultSet rs, Schema writeSchema)
        throws SQLException {
     ...
     * @throws SQLException
     */
    private static String getRecordPartitionPath(ResultSet rs, Schema writeSchema)
        throws SQLException {
     ...
     * @throws SQLException
     */
    private static String getNestedFieldValAsString(ResultSet rs, Schema writeSchema,
        String fieldName) throws SQLException {
        Object value = null;
        if (writeSchema.getFields().stream().map(field -> field.name()).collect(Collectors.toList())
            .contains(fieldName)) {
            value = convertValueType(rs, fieldName,
     ...
     * @throws SQLException
     */
    protected static Object convertValueType(ResultSet rs, String name, Schema.Type dataType)
        throws SQLException {
        Object value = null;
        if (dataType != null) {
     ...
     * @throws IOException
     */
    public static void authenticate(String localKeytab, String principal, String krb5Str)
        throws IOException {
    ...
    public static Schema getJDBCSchema(Map<String, String> options) throws Exception {
        ...
        try (final Statement confStatement = conn.createStatement();
            PreparedStatement schemaQueryStatement = conn.prepareStatement(
    ...
    private static Connection createConnectionFactory(Map<String, String> options)
        ...
        while (drivers.hasMoreElements()) {
            ...
            if (d instanceof DriverWrapper) {
                if (((DriverWrapper) d).wrapped().getClass().getCanonicalName()
                    .equals(driverClass)) {
                    driver = d;
                }
        ...
        Objects.requireNonNull(driver,
            String.format("Did not find registered driver with class %s", driverClass));
