...
public class HoodieExampleDataGenerator<T extends HoodieRecordPayload<T>> {
  ...
  public List<HoodieRecord<T>> generateInserts2(String commitTime, Integer n) {
    return generateInsertsStream2(commitTime, n).collect(Collectors.toList());
  }

  public Stream<HoodieRecord<T>> generateInsertsStream2(String commitTime, Integer n) {
    int currSize = getNumExistingKeys();

    return IntStream.range(0, n).boxed().map(i -> {
      String partitionPath = DEFAULT_SECOND_PARTITION_PATH;//partitionPaths[RAND.nextInt(partitionPaths.length)];
      HoodieKey key = new HoodieKey(i + "_key", partitionPath);
